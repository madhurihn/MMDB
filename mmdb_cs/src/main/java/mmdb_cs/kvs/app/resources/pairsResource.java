package mmdb_cs.kvs.app.resources;
...
import mmdb_cs.kvs.app.output.FineGrainedPair;
import mmdb_cs.kvs.app.output.FineGrainedPairs;
...
public class pairsResource {
	...
	public static ObjectObjectAssociativeContainer<String, Map<String, String>> map ;
    ...
    public pairsResource() {
      	...
      	this.map = new ObjectObjectHashMap<String,Map<String,String>>();
    ...
    public Response getAllPaFineGrainedPairs() {
    		...
    		//for (ObjectCursor<String> entry : map.keys())
    		//for (Map.Entry<String, Map<String, String>> entry : map.entrySet())
    		for (ObjectObjectCursor entry : map)
    		{
    			FineGrainedPair tuple = new FineGrainedPair();
    			
    			tuple.setKey((String) entry.key);
    			Map<String, String> testMap = (Map<String, String>) entry.value;
    			tuple.setField0(((String) testMap.get("field0")));
    			tuple.setField1(((String)testMap.get("field1")));
    			tuple.setField2(((String)testMap.get("field2")));
    			tuple.setField3(((String)testMap.get("field3")));
    			tuple.setField4(((String)testMap.get("field4")));
    			tuple.setField5(((String)testMap.get("field5")));
    			tuple.setField6(((String)testMap.get("field6")));
    			tuple.setField7(((String)testMap.get("field7")));
    			tuple.setField8(((String)testMap.get("field8")));
    			tuple.setField9(((String)testMap.get("field9")));
    ...
    public Response getPair(@PathParam(value="key") String key, @QueryParam(value="table") String table) {
    	FineGrainedPair tuple = new FineGrainedPair();
    		if (map.containsKey(key)) {
    			Map<String, String> entry= ((ObjectObjectHashMap<String,Map<String,String>>) map).get(key);
    			tuple.setKey(key);
...
//			return Response.status(Status.BAD_REQUEST).build();
		//}
		try{		
			((ObjectObjectHashMap<String,Map<String,String>>) map).put(pair.getKey(), value);
		}
		catch(Exception e){
	...
	String table, @QueryParam(value="key")String key) {
	   ...
	   if (!map.containsKey(key)){
		...
		try{		
			((ObjectObjectHashMap<String,Map<String,String>>) map).remove(key);
		}
		catch(Exception e){
			return Response.status(Status.INTERNAL_SERVER_ERROR).build();
		}
